...
COMMENT ON COLUMN vzd.adreses_pp.ppils_cd IS 'Priekšpilsētas adreses kods.';

COMMENT ON COLUMN vzd.adreses_pp.dat_sak IS 'Sasaistes izveidošanas datums.';

COMMENT ON COLUMN vzd.adreses_pp.dat_mod IS 'Datums, kad pēdējo reizi informācijas sistēmā tehniski modificēts ieraksts/dati par sasaisti.';

COMMENT ON COLUMN vzd.adreses_pp.dat_beig IS 'Sasaistes likvidācijas datums, ja sasaiste beidza pastāvēt.';
...
--Dokumenti, uz kuru pamata reģistrētas vai aktualizētas adreses.
DROP TABLE IF EXISTS vzd.adreses_doc;

CREATE TABLE vzd.adreses_doc (
  id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
...
COMMENT ON TABLE vzd.adreses_doc IS 'Dokumenti, uz kuru pamata reģistrētas vai aktualizētas adreses.';

COMMENT ON COLUMN vzd.adreses_doc.id IS 'ID.';

COMMENT ON COLUMN vzd.adreses_doc.adreses_kods IS 'Adresācijas objekta kods.';

COMMENT ON COLUMN vzd.adreses_doc.autors IS 'Dokumenta autors.';

COMMENT ON COLUMN vzd.adreses_doc.datums IS 'Dokumenta datums.';

COMMENT ON COLUMN vzd.adreses_doc.veids IS 'Dokumenta veids.';

COMMENT ON COLUMN vzd.adreses_doc.numurs IS 'Dokumenta numurs.';

COMMENT ON COLUMN vzd.adreses_doc.nosaukums IS 'Dokumenta nosaukums.';

--Papildina dokumentu veidu klasifikatoru.
WITH c
AS (
  SELECT veids
  FROM aw_csv.aw_doc_nl
  
  UNION
  
  SELECT veids
  FROM aw_csv.aw_doc_tg
  
  UNION
  
  SELECT veids
  FROM aw_csv.aw_doc_vieta
  )
INSERT INTO vzd.adreses_doc_veids (veids)
SELECT veids
FROM c
WHERE veids NOT IN (
    SELECT veids
    FROM vzd.adreses_doc_veids
    )
ORDER BY veids;

---Ēkas un zemes vienības.
INSERT INTO vzd.adreses_doc (
  adreses_kods
  ,autors
  ,datums
  ,veids
