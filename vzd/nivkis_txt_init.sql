...
COMMENT ON COLUMN vzd.nivkis_valuation_property."ValueType" IS 'Novērtējuma veida kods.';
...
--ValueType klasifikators.
DROP TABLE IF EXISTS vzd.nivkis_valuation_type;

CREATE TABLE vzd.nivkis_valuation_type (
  id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
  ,"ValueType" TEXT NOT NULL
  ,"ValDescription" TEXT NOT NULL
  );

COMMENT ON TABLE vzd.nivkis_valuation_type IS 'Novērtējuma veidu klasifikators Nekustamā īpašuma valsts kadastra informācijas sistēmas atvērtajos teksta datos.';

COMMENT ON COLUMN vzd.nivkis_valuation_type.id IS 'Kods.';

COMMENT ON COLUMN vzd.nivkis_valuation_type."ValueType" IS 'Kods.';

COMMENT ON COLUMN vzd.nivkis_valuation_type."ValDescription" IS 'Nosaukums.';

GRANT SELECT, INSERT, UPDATE
  ON TABLE vzd.nivkis_valuation_type
  TO scheduler;

GRANT SELECT, UPDATE
  ON SEQUENCE vzd.nivkis_valuation_type_id_seq
  TO scheduler;

ALTER TABLE vzd.nivkis_valuation_property ADD CONSTRAINT "nivkis_valuation_property_fk_ValueType" FOREIGN KEY ("ValueType") REFERENCES vzd.nivkis_valuation_type (id);

ALTER TABLE vzd.nivkis_valuation_object ADD CONSTRAINT "nivkis_valuation_object_fk_ValueType" FOREIGN KEY ("ValueType") REFERENCES vzd.nivkis_valuation_type (id);

--nivkis_valuation_forest.
DROP TABLE IF EXISTS vzd.nivkis_valuation_forest;

CREATE TABLE vzd.nivkis_valuation_forest (
  id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
  ,"ObjectCadastreNr" VARCHAR(17) NOT NULL
  ,"ForestValue" INT
...
COMMENT ON TABLE vzd.nivkis_valuation_forest IS 'Nekustamā īpašuma valsts kadastra informācijas sistēmas atvērtie teksta dati par mežaudžu vērtībām.';

COMMENT ON COLUMN vzd.nivkis_valuation_forest.id IS 'ID.';

COMMENT ON COLUMN vzd.nivkis_valuation_forest."ObjectCadastreNr" IS 'Objekta kadastra apzīmējums.';

COMMENT ON COLUMN vzd.nivkis_valuation_forest."ForestValue" IS 'Mežaudzes vērtība, €.';

COMMENT ON COLUMN vzd.nivkis_valuation_forest."ForestValueDate" IS 'VMD datu sagatavošanas datums.';

COMMENT ON COLUMN vzd.nivkis_valuation_forest.date_created IS 'Izveidošanas datums. Atbilst datu kopā norādītajam datējumam.';

COMMENT ON COLUMN vzd.nivkis_valuation_forest.date_deleted IS 'Dzēšanas datums. Atbilst vecākajā datu kopā, kurā vairs nav objekta ar šādiem atribūtiem, norādītajam datējumam.';

GRANT SELECT, INSERT, UPDATE
  ON TABLE vzd.nivkis_valuation_forest
  TO scheduler;

GRANT SELECT, UPDATE
  ON SEQUENCE vzd.nivkis_valuation_forest_id_seq
  TO scheduler;